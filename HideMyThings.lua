                        ...
                        if btn and not btn.hooked then
                            ...
                            for _, name in ipairs(microList) do
                                ...
                                if btn and not btn.hooked then
                                    ...
                                    -- 1. Elements (PlayerFrame, Resources, etc.)
                                    for _, data in ipairs(elements) do
                                        local frame = _G[data.name]
                                        if frame then
                                            local isMouseOver = frame:IsMouseOver()

                                            -- Logical Adjustment: Only PlayerFrame and MyResourcesFrame respond to "hasTarget"
                                            local respondsToTarget = (data.name == "PlayerFrame" or data.name == "MyResourcesFrame")
                                            local shouldShow = (data.combatLinked and (inCombat or isMouseOver or (respondsToTarget and hasTarget)))
                                            or (not data.combatLinked and isMouseOver)

                                            if shouldShow then
                                                data.linger = 0; data.targetAlpha = 1
                                                else
                                                    data.linger = data.linger + elapsed;
                                                data.targetAlpha = (data.linger >= LINGER_DELAY) and data.idleAlpha or 1
                                                end
                                                ...
                                                if data.currentAlpha ~= data.targetAlpha then
                                                    ...
                                                    for _, btn in ipairs(Bar1.buttons) do if btn and btn:IsMouseOver() then b1M = true break end end
                                                        ...
                                                        -- Removed "hasTarget" from Bar 1 trigger
                                                        local sB1 = inCombat or b1M

                                                        if sB1 then
                                                            Bar1.linger = 0; Bar1.targetAlpha = 1
                                                            else
                                                                Bar1.linger = Bar1.linger + elapsed;
                                                            Bar1.targetAlpha = (Bar1.linger >= LINGER_DELAY) and Bar1.idleAlpha or 1
                                                            end
                                                            ...
                                                            if Bar1.currentAlpha ~= Bar1.targetAlpha then
                                                                ...
                                                                for _, btn in ipairs(Bar1.buttons) do if btn then btn:SetAlpha(Bar1.currentAlpha) end end
                                                                    if MainMenuBar then MainMenuBar:SetAlpha(Bar1.currentAlpha) end
                                                                        ...
                                                                        -- 3. Micro/Bags Cluster (Remains mouse-over only as per your original logic)
                                    ...
                                    if not micM then for _, btn in ipairs(Micro.buttons) do if btn:IsMouseOver() then micM = true break end end end
                                        if micM then
                                            Micro.linger = 0; Micro.targetAlpha = 1
                                            else
                                                Micro.linger = Micro.linger + elapsed;
                                            Micro.targetAlpha = (Micro.linger >= LINGER_DELAY) and Micro.idleAlpha or 1
                                            end
