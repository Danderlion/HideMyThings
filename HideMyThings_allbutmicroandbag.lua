...
local elements = {
    ...
    { name = "ObjectiveTrackerFrame", idleAlpha = 0.05, combatLinked = false },
...
for _, data in ipairs(elements) do
    ...
    for i = 1, 12 do table.insert(Bar1.buttons, _G["ActionButton"..i]) end
        ...
        -- Cluster 2: Micro/Bags (Extracted from MAS Logic)
        local Micro = { currentAlpha = 0, targetAlpha = 0, idleAlpha = 0, linger = 0, combatLinked = false, buttons = {} }
        local microList = {
            "CharacterMicroButton", "ProfessionMicroButton", "PlayerSpellsMicroButton", "AchievementMicroButton",
            "QuestLogMicroButton", "GuildMicroButton", "LFDMicroButton", "CollectionsMicroButton",
            "EJMicroButton", "StoreMicroButton", "MainMenuMicroButton", "MainMenuBarBackpackButton"
        }
        for i = 0, 3 do table.insert(microList, "CharacterBag"..i.."Slot") end
            for _, name in ipairs(microList) do if _G[name] then table.insert(Micro.buttons, _G[name]) end end
                ...
                for _, data in ipairs(elements) do
                    ...
                    if frame and not frame.hooked then
                        ...
                        -- Granular Button Hooks for Bar 1
                        for _, btn in ipairs(Bar1.buttons) do
                            if btn and not btn.hooked then
                                ...
                                -- Granular Button Hooks for Micro/Bags
                                for _, btn in ipairs(Micro.buttons) do
                                    if btn and not btn.hooked then
                                        hooksecurefunc(btn, "SetAlpha", function(self, a)
                                                                        ...
                                                                        if Bar1.currentAlpha ~= Bar1.targetAlpha then
                                                                            ...
                                                                            for _, btn in ipairs(Bar1.buttons) do if btn then btn:SetAlpha(Bar1.currentAlpha) end end
                                                                                if MainMenuBar then MainMenuBar:SetAlpha(Bar1.currentAlpha) end
                                                                                    ...
                                                                                    -- 3. Micro/Bags Cluster (Mouseover Only)
                                        local micM = false
                                        if MicroButtonAndBagsBar and MicroButtonAndBagsBar:IsMouseOver() then micM = true
                                            else for _, btn in ipairs(Micro.buttons) do if btn and btn:IsMouseOver() then micM = true break end end end
